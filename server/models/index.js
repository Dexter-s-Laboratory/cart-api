...
module.exports = {
  ...
  createTransactionInDB: async (buyer_id, total_amount, listing_ids) => {
    ...
    try {
      ...
      return [insertTransaction, updateListing];
    } catch(error) {
      console.log('unable to retrieve listings, ', error);
    }
  },
  ...
  getPurchasesByUserIdFromDB: async(user_id) => {
    try {
      const purchases  = await db.any('SELECT listings.* FROM listings JOIN transactions WHERE transactions.id = listings.transactions_id');
      return purchases;
    } catch(error) {
  ...
  getSalesByUserIdFromDB: async (user_id) => {
    try {
      const sales = await db.any('SELECT * from listings where seller_id = $1 AND status = $2',[user_id, 'fulfilled'] )
      return sales;
    } catch(error) {
